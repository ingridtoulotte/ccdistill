...
**Does ccrecall send my data anywhere?**
...
`/context` measures the live session, including version-specific system prompt changes and the true MCP tool schemas. ccrecall estimates statically without burning a session. Expect agreement on the controllable parts (CLAUDE.md, memory) and drift on baselines and MCP. Trend and recommendations are the point, not the third significant digit.

**Are distill suggestions safe to paste directly?**
Review them first — they're heuristic. The point is surfacing what you keep repeating; phrasing durable rules is your call (or `ccrecall distill --prompt | claude -p` to have Claude draft them).

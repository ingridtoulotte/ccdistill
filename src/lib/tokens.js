 ...
 * Real tokenizers vary ±20%; ccrecall labels every estimate as such.
 */
 ...
 * wins. Overridable via ~/.ccrecall.json: { "pricing": [{ "match": "...",
...
function loadConfig() {
  ...
  const file = process.env.CCRECALL_CONFIG || path.join(os.homedir(), '.ccrecall.json');
  try {

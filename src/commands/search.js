...
module.exports = async function searchCmd(ctx) {
  ...
  if (!query) {
    throw new Error('usage: ccrecall search <query> [--regex] [--role user|assistant] [--project x] [--since 30d] [--limit n]');
  }
  ...
  out.push('');
  out.push(c.dim(`${matches.length} match(es). Open a session with: ccrecall show <id>`));
